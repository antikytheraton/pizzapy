...
ordenes = ['ALITAS_BARBACOA','ALITAS_CAYENNE','DON_CALZONE_DELUXE','DON_CALZONE_TROPICAL','CHESSY_BREAD','COCA_COLA','SPRITE','FANTA','5CARNES_CHICA','5CARNES_GRANDE','5CARNES_MEGA','HAWAIANA_CHICA','HAWAIANA_GRANDE','HAWAIANA_MEGA','CHEESE_CHICA','CHEESE_GRANDE','CHEESE_MEGA','CARNEYTOCINO_CHICA','CARNEYTOCINO_GRANDE','CARNEYTOCINO_MEGA','AMERICANA_CHICA','AMERICANA_GRANDE','AMERICANA_MEGA']
# list_of_items = []
...
def received_message(event):
    ...
    if quick_reply_payload == 'INIT':
        menu_inicio(sender_id)
...
def received_postback(event):
    ...
    if payload == 'MENU_PIZZAS':
        '''Menu Pizzas'''
        menu_pizzas(sender_id)

    if payload == 'MENU_EXTRAS':
        '''Menu Extras'''
        menu_extras(sender_id)

    if payload in ordenes:
        '''Seleccion de pedidos'''
        nombre_orden, tipo_orden = nombre_producto(sender_id, payload)
        pregunta_cantidad(sender_id, nombre_orden, tipo_orden)

    if payload == 'CONFIRMAR':
        '''Confirmar'''
        confirmar_operacion(sender_id)

    if payload == 'CANCELAR':
        '''Cancelar'''
        cancelar_operacion(sender_id)
